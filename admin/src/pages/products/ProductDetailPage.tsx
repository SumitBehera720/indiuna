...
import { ArrowLeft, Package, Pencil, X, Upload, Image as ImageIcon, Star, Loader2, Trash2 } from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  const openEditModal = () => {
    ...
    const catId = mainCat ? mainCat.id : '';
    ...
    if (parsedVariants.length === 0) {
      const sku = product.variants?.[0]?.sku || (product.name.split(' ')[0].toUpperCase() + '-' + Math.random().toString(36).substr(2, 5).toUpperCase());
      parsedVariants.push({
        id: undefined,
        sku,
    ...
    const hasVarOptions = parsedVariants.some(v => v.attributes && (v.attributes.color || v.attributes.size));
    setHasOptions(hasVarOptions);
    ...
    setEditForm({
      ...
      size_guide_image: (product as any).size_guide_image || '',
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    const availableSlots = 5 - editForm.images.length;
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        if (url) {
          uploaded.push({
            url,
            is_primary: editForm.images.length === 0 && i === 0,
          });
        }
      }
  ...
  const handleSizeGuideUpload = async (file: File | null) => {
    ...
    try {
      ...
      const url = data.url || data.data?.url;
      if (url) {
        setEditForm(prev => ({ ...prev, size_guide_image: url }));
  ...
  const addColorGroup = (colorName: string, colorCode: string = '#333333') => {
    ...
    const exists = editForm.variants.some(v => v.attributes?.color?.toLowerCase() === cleanColor.toLowerCase());
    if (exists) {
      toast.error(`Color "${cleanColor}" already exists`);
      return;
    }
    const basePrice = editForm.variants[0]?.price || 999;
    const baseCompare = editForm.variants[0]?.compare_price || null;
    const baseStock = editForm.variants[0]?.stock || 10;

    // Automatically generate standard size variants for the new color
    const defaultSizes = ['S', 'M', 'L', 'XL', 'XXL'];
    const newVariants = defaultSizes.map(size => ({
      sku: generateSKU(editForm.name, cleanColor, size),
      price: basePrice,
  ...
  const addSizeToColor = (color: string, sizeName: string) => {
    ...
    if (!cleanSize) return;
    const exists = editForm.variants.some(v => v.attributes?.color === color && v.attributes?.size?.toLowerCase() === cleanSize.toLowerCase());
    if (exists) {
      toast.error(`Size "${cleanSize}" already exists for ${color}`);
      return;
    }
    const colorVars = editForm.variants.filter(v => v.attributes?.color === color);
    ...
    const newVar = {
      sku: generateSKU(editForm.name, color, cleanSize),
      price: basePrice,
      compare_price: baseCompare,
      stock: baseStock,
      is_active: true,
  ...
  const toggleSizeInColor = (color: string, size: string) => {
    const exists = editForm.variants.some(v => v.attributes?.color === color && v.attributes?.size === size);
    if (exists) {
      setEditForm(prev => ({
        ...prev,
        variants: prev.variants.filter(v => !(v.attributes?.color === color && v.attributes?.size === size))
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (hasOptions) {
      const activeVars = editForm.variants.filter(v => v.is_active && v.attributes?.size);
      ...
      const baseVar = editForm.variants[0];
      if (!baseVar) return toast.error('No variant data found');
      if (!baseVar.sku?.trim()) return toast.error('SKU is required');
    ...
    try {
      ...
      const formattedImages = editForm.images.map((img, idx) => ({
        url: img.url,
        is_primary: img.is_primary || (idx === 0),
        sort_order: idx,
        alt_text: editForm.name,
      }));
      ...
      const payload = {
        ...
        size_guide_image: editForm.size_guide_image || null,
        categories: [editForm.categoryId].filter(Boolean),
        tags: [editForm.fitType].filter(Boolean),
        variants: submittedVariants,
        images: formattedImages
      };

      const updated = await productService.update(product.id, payload as any);
      setProduct(updated);
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      const updated = await productService.update(product.id, { status: newStatus });
      setProduct(updated);
    ...
    return (
      <div>
        <div className="flex items-center gap-4 mb-6">
          <div className="w-9 h-9 rounded-xl bg-surface-100 dark:bg-surface-800 animate-pulse" />
          <div className="h-7 w-48 bg-surface-100 dark:bg-surface-800 rounded animate-pulse" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <button onClick={() => navigate('/products')} className="flex items-center gap-2 text-sm text-surface-500 hover:text-surface-700 dark:hover:text-surface-300 mb-4 transition-colors">
        <ArrowLeft size={16} />
        Back to Products
      </button>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-surface-500 mt-1">SKU: {product.variants?.[0]?.sku || 'N/A'} &middot; Created {formatDate(product.created_at)}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={openEditModal}
            className="px-4 py-2 rounded-xl text-sm font-medium bg-surface-100 hover:bg-surface-200 dark:bg-surface-850 dark:hover:bg-surface-800 text-surface-700 dark:text-surface-300 transition-colors flex items-center gap-2"
          >
            <Pencil size={15} />
            Edit Product
          </button>
          ...
          >
            {togglingStatus ? 'Updating...' : product.status === 'published' ? 'Set as Draft' : 'Publish'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="card p-6">
            <h2 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-4">Basic Information</h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-surface-400 mb-1">Price</p>
                <p className="font-medium text-surface-900 dark:text-white">{formatCurrency(product.price)}</p>
              </div>
              <div>
                <p className="text-xs text-surface-400 mb-1">Compare Price</p>
                <p className="font-medium text-surface-900 dark:text-white">{product.compare_price ? formatCurrency(product.compare_price) : '—'}</p>
              </div>
              <div>
                <p className="text-xs text-surface-400 mb-1">Stock</p>
                <p className="font-medium text-surface-900 dark:text-white">{product.stock}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-surface-900 dark:text-white">{product.categories?.[0]?.name || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-surface-400 mb-1">Featured</p>
                <p className="font-medium text-surface-900 dark:text-white">{product.is_featured ? 'Yes' : 'No'}</p>
              </div>
            </div>
          </div>
          ...
          {product.description && (
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-4">Description</h2>
              <p className="text-sm text-surface-700 dark:text-surface-300 leading-relaxed">{product.description}</p>
            </div>
          )}

          <div className="card p-6">
            <h2 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-4">Variants ({product.variants?.length || 0})</h2>
            {product.variants?.length ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-surface-100 dark:border-surface-700/50">
                      <th className="text-left py-2 px-2 text-surface-500 font-medium">SKU</th>
                      <th className="text-left py-2 px-2 text-surface-500 font-medium">Price</th>
                      <th className="text-left py-2 px-2 text-surface-500 font-medium">Stock</th>
                      <th className="text-left py-2 px-2 text-surface-500 font-medium">Attributes</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-surface-100 dark:divide-surface-700/50">
                    {product.variants.map((v) => (
                      <tr key={v.id} className="hover:bg-surface-50 dark:hover:bg-surface-800/50">
                        <td className="py-2 px-2 font-medium text-surface-900 dark:text-white">{v.sku}</td>
                        <td className="py-2 px-2">{formatCurrency(v.price)}</td>
                        <td className="py-2 px-2">{v.stock}</td>
                        <td className="py-2 px-2 text-surface-500">{Object.entries(v.attributes || {}).map(([k, val]) => `${k}: ${val}`).join(', ') || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="space-y-6">
          <div className="card p-6">
            <h2 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-4">Images</h2>
            {product.images?.length ? (
              <div className="grid grid-cols-2 gap-2">
                {product.images
                  .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0))
                  .map((img) => (
                    <div key={img.id} className={cn('relative rounded-lg overflow-hidden border-2', img.is_primary ? 'border-primary-500' : 'border-transparent')}>
                      <img src={img.url} alt={product.name} className="w-full aspect-square object-cover" />
                      {img.is_primary && <span className="absolute top-1 left-1 text-[10px] px-1.5 py-0.5 rounded bg-primary-500 text-white font-medium">Primary</span>}
                    </div>
                  ))}
              </div>
            ) : (
          ...
          {(product as any).size_guide_image && (
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-4">Size Guide Image</h2>
              <div className="rounded-xl overflow-hidden border border-surface-200 dark:border-surface-800">
                <img src={(product as any).size_guide_image} alt="Size Guide" className="w-full object-cover" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-surface-900 rounded-2xl w-full max-w-xl border border-surface-200 dark:border-surface-800 p-6 shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-surface-900 dark:text-white">Edit Product</h2>
              <button onClick={() => setShowEditModal(false)} className="text-surface-400 hover:text-surface-500 dark:hover:text-surface-300">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleEditSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={editForm.fitType}
                    onChange={e => setEditForm(prev => ({ ...prev, fitType: e.target.value }))}
                    className="w-full rounded-xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-950 px-4 py-2.5 text-sm text-surface-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500"
                  >
              ...
              <div className="p-4 bg-surface-50 dark:bg-surface-850 rounded-2xl border border-surface-200 dark:border-surface-800 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2">
                  <div>
                    ...
                    <input
                      ...
                      required
                      min={0}
                      value={editForm.variants[0]?.price || ''}
                      onChange={e => {
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      min={0}
                      value={editForm.variants[0]?.compare_price || ''}
                      onChange={e => {
                    ...
                    <>
                      <div>
                        ...
                        <input
                          ...
                          required
                          min={0}
                          value={editForm.variants[0]?.stock ?? 0}
                          onChange={e => {
                            const val = Number(e.target.value);
                            setEditForm(prev => {
                ...
                {hasOptions && (
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="flex flex-col gap-3 p-3 bg-surface-55 dark:bg-surface-950 rounded-xl border border-surface-200 dark:border-surface-800">
                    ...
                    <div className="space-y-5 max-h-[40vh] overflow-y-auto pr-1">
                      {Array.from(new Set(editForm.variants.map(v => v.attributes?.color || 'Default'))).map(color => {
                        const colorVars = editForm.variants.filter(v => (v.attributes?.color || 'Default') === color);
                        return (
                              ...
                              <table className="w-full text-xs">
                                ...
                                <tbody className="divide-y divide-surface-100 dark:divide-surface-800">
                                  {colorVars.filter(v => v.attributes?.size).map(v => (
                                    <tr key={`${color}-${v.attributes?.size}`} className="hover:bg-surface-50/50 dark:hover:bg-surface-950/50">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-surface-400">{editForm.images.length}/5 uploaded</span>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <label className="flex-1 border border-surface-200 dark:border-surface-800 rounded-xl p-3 flex items-center justify-center gap-2 cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-850 transition-colors">
                    ...
                    <span className="text-xs font-semibold text-surface-700 dark:text-surface-300">
                      {editForm.size_guide_image ? 'Change Size Guide File' : 'Upload Size Guide File from Device'}
                    </span>
                    <input
                      type="file"
                      accept="image/*"
                  ...
                  {editForm.size_guide_image && (
                    <div className="relative w-16 h-16 rounded-xl overflow-hidden border border-surface-200 dark:border-surface-800 shrink-0">
                      <img src={editForm.size_guide_image} alt="Size Guide" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => setEditForm(prev => ({ ...prev, size_guide_image: '' }))}
